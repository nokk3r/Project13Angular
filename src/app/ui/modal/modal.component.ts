...
export class ModalComponent implements OnInit {
  ...
  onEditWorker(id:number, name: string, surname: string, number: string){
    this.editWorker.emit({id,name,surname, number})
  }
  ...
  close(modal){
    if(this.name != '' && this.surname != '' && this.number != ''){
      this.onEditWorker(this.id, this.editWorkerForm.value.newName, this.editWorkerForm.value.newSurname, this.editWorkerForm.value.newNumber)
      modal.close()
    }
  }
